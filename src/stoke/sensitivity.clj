;;OBE? What's this about....?
(ns stoke.sensitivity
  (:require [helmet [core :as helm]  [split :as split]] ;;helmet dependency.
            [stoke [io :as stokeio] [scraper :as scraper]
                   [app :as    app] [core :as stoke]]
            [spork.util [table :as tbl]]))

(def records->peak-records
  (comp stokeio/peaks->records stokeio/table->peaks))

;;This is a quick hack to facilitate generating one or more splits based off of
;;a demand future case. Given a set of demand splits, we use the same demand
;;split map defined in the workbook, and basically just tweak the parameters.
(defn demand-sensitivity
  [wbpath & {:keys [splits peaks?]
             :or {splits [60 90 180 360] peaks? nil}}]
...
;;A simple supply sensitivity analysis. We compute multiple supplies relative to
;;the

(defn build-sensitivity-portfolio [project finite-demands]
  (assert (vector? finite-demands)
          "cannot pass a potentially infinite data structure")
  ...
  (let [n              (count finite-demands)
    ...
    (map (fn [[case-name fut]]
           (let [res (hfill fut :supply supply)]
             [case-name {:supply (:supply res) :fills (:fills res)}]))
         finite-demands)))
...
(defn spit-supplies [outfolder supply-pairs]
  (let [fillpath (str outfolder "\\fills\\")
    ...
    (doseq [[case {:keys [supply fills]}] supply-pairs]
      (do (spork.util.io/hock (str fillpath (case->filename case) ".txt")
                              (app/fills->txt fills))
          (spork.util.io/hock (str supplypath (case->filename case) ".txt")
                              (app/supply->txt supply))))))
...
(defn spit-supplies-rolled [outfolder supply-pairs]
  (let [fillpath   (str outfolder "\\fills\\fills.txt")
        ...
        fills      (map (fn [[case m]] [(case->filename case)
                                        (app/fills->table  (:fills m))])
                        supply-pairs)
        supplies   (map (fn [[case m]] [(case->filename case)
                                        (app/supply->table (:supply m))])
                        supply-pairs)]
    (do (spork.util.io/hock fillpath
            (tbl/table->tabdelimited (roll-tables fills :SensitivityCase)))
        (spork.util.io/hock supplypath
            (tbl/table->tabdelimited (roll-tables supplies :SensitivityCase))))))
